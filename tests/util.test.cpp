...
static_assert(
    scribe::Handler<scribe::util::Filter<CountingHandler,
                                         decltype([](const Record<int>&) -> bool { return true; })>,
                    int>);
...
TEST(Chain, ShortCircuitsWhenHandlerReturnsFalse) {
    int                 a = 0;
    int                 b = 0;
    scribe::util::Chain chain{
        GatingHandler{.count = &a, .result = false},
        CountingHandler{&b}
    };
    chain.handle(make_record(0));
    EXPECT_EQ(a, 1);
...
TEST(Chain, ContinuesWhenHandlerReturnsTrue) {
    ...
    int                 b = 0;
    scribe::util::Chain chain{
        GatingHandler{.count = &a, .result = true},
        CountingHandler{&b}
    };
    chain.handle(make_record(0));
    EXPECT_EQ(a, 1);
...
TEST(Filter, ForwardsWhenPredicateTrue) {
    int                  count = 0;
    scribe::util::Filter filter{CountingHandler{&count},
                                [](const Record<int>&) -> bool { return true; }};
...
TEST(Filter, DropsAndReturnsFalseWhenPredicateFalse) {
    int                  count = 0;
    scribe::util::Filter filter{CountingHandler{&count},
                                [](const Record<int>&) -> bool { return false; }};
    auto                 result = filter.handle(make_record(0));
    EXPECT_EQ(count, 0);
    EXPECT_FALSE(result);
}
...
TEST(Filter, PredicateReceivesRecord) {
    int                  dummy = 0;
    int                  seen  = -1;
    scribe::util::Filter filter{CountingHandler{&dummy}, [&](const Record<int>& r) -> bool {
                                    seen = r.payload;
                                    return true;
                                }};
